...
/** @mainpage wxMathPlot
   ...
   can be function plots, scale rulers, or any other vector data visualisation.
   ...
   mpLegend and so on. These base classes already come with plot code, user's
   own functions can be implemented by overriding just one member for retrieving
   a function value.
...
class WXDLLIMPEXP_MATHPLOT mpLayer : public wxObject
{
   public:
	...
	/** Check whether this layer has a bounding box.
		...
		FALSE if your mpLayer implementation should be ignored by the
	   calculation of the global bounding box for all layers in a mpWindow.
	...
	*/
	...
	/** Check whether the layer is an info box.
	   ...
	   for mpInfoLayer class and its derivative. It is necessary to define mouse
	   actions behaviour over info boxes.
	...
	*/
	...
	pixel coordinates, and <b>user code should rely on them</b> for portability
	and future changes to be applied transparently, instead of implementing the
	following formulas manually.
	...
	orientation for mathematical plots is vice versa. Thus Y-orientation will be
	swapped usually, when transforming between wxDC and mpLayer coordinates.
	This change of coordinates is taken into account in the methods
	p2x,p2y,x2p,y2p.
		<b> Rules for transformation between mpLayer and wxDC coordinates </b>
		@code
		dc_X = (layer_X - mpWindow::GetPosX()) * mpWindow::GetScaleX()
		dc_Y = (mpWindow::GetPosY() - layer_Y) * mpWindow::GetScaleY() //
	swapping Y-orientation
	...
	*/
	 ...
	 * line, false:draws separate points).
	 * @sa GetContinuity
	 */
	...
	*/
	...
	/** Set Draw mode: inside or outside margins. Default is outside, which
	   allows the layer to draw up to the mpWindow border.
   ...
   protected:
	...
	/** Layer's name */
	wxString m_name;
	/** Specify if the layer will be plotted as a continuous line or a set of
	 * points. */
	bool m_continuous;
...
class WXDLLIMPEXP_MATHPLOT mpInfoLayer : public mpLayer
{
   ...
   protected:
	/** The bounding rectangle of the box. It may be resized dynamically by the
	 * Plot method. */
	wxRect m_dim;
	...
	wxBrush m_brush;
	/** Holds the mpWindow size. Used to rescale position when window is
	 * resized. */
	int m_winX, m_winY;
...
/** @class mpInfoCoords
   ...
   units. When an mpInfoCoords layer is activated, when mouse is moved over the
...
/** Abstract base class providing plot and labeling functionality for functions
   F:X->Y. Override mpFX::GetY to implement a function. Optionally implement a
   constructor and pass a name (label) and a label alignment to the constructor
   mpFX::mpFX. If the layer name is empty, no label will be plotted.
*/
class WXDLLIMPEXP_MATHPLOT mpFX : public mpLayer
{
   public:
	/** @param name  Label
		@param flags Label alignment, pass one of #mpALIGN_RIGHT,
	   #mpALIGN_CENTER, #mpALIGN_LEFT.
	*/
...
/** Abstract base class providing plot and labeling functionality for functions
   F:Y->X. Override mpFY::GetX to implement a function. Optionally implement a
   constructor and pass a name (label) and a label alignment to the constructor
   mpFY::mpFY. If the layer name is empty, no label will be plotted.
*/
...
/** Abstract base class providing plot and labeling functionality for a locus
   ...
   and mpFXY::GetNextXY to implement a locus. Optionally implement a constructor
   and pass a name (label) and a label alignment to the constructor
...
/** Abstract base class providing plot and labeling functionality for functions
   F:Y->X. Override mpProfile::GetX to implement a function. This class is
   similar to mpFY, but the Plot method is different. The plot is in fact
   represented by lines instead of points, which gives best rendering of
   rapidly-varying functions, and in general, data which are not so close one to
   another. Optionally implement a constructor and pass a name (label) and a
   label alignment to the constructor mpProfile::mpProfile. If the layer name is
   empty, no label will be plotted.
*/
   ...
   public:
	/** Full constructor.
	   ...
	   ticks, FALSE for drawing the grid.
		@param type mpX_NORMAL for normal labels, mpX_TIME for time axis in
	   hours, minutes, seconds. */
	mpScaleX(
		wxString name = wxT("X"), int flags = mpALIGN_CENTER, bool ticks = true,
	...
	/** Set X axis label view mode.
		@param mode mpX_NORMAL for normal labels, mpX_TIME for time axis in
	   hours, minutes, seconds. */
...
/** Plot layer implementing a y-scale ruler.
   ...
   plotted at the top-right hand of the ruler. The scale numbering automatically
	adjusts to view and zoom factor.
*/
class WXDLLIMPEXP_MATHPLOT mpScaleY : public mpLayer
{
   public:
	/** @param name Label to plot by the ruler
	   ...
	   ticks, FALSE for drawing the grid */
	mpScaleY(
		wxString name = wxT("Y"), int flags = mpALIGN_CENTER,
		bool ticks = true);
	...
	*/
	...
	/** Set Y axis alignment.
		@param align alignment (choose between mpALIGN_BORDER_LEFT,
	   mpALIGN_LEFT, mpALIGN_CENTER, mpALIGN_RIGHT, mpALIGN_BORDER_RIGHT) */
	void SetAlign(int align) { m_flags = align; };
...
/** Canvas for plotting mpLayer implementations.
   ...
   view. The context menu can be retrieved with mpWindow::GetPopupMenu, e.g. for
   extending it externally.
...
class WXDLLIMPEXP_MATHPLOT mpWindow : public wxWindow
{
	...
	*/
	...
	/** Remove a plot layer from the canvas.
		@param layer Pointer to layer. The mpLayer object will be destructed
	   using delete.
	   ...
	   after removing the layer.
		...
		N.B. Only the layer reference in the mpWindow is deleted, the layer
	   object still exists!
	*/
	bool DelLayer(
		mpLayer* layer, bool alsoDeleteObject = false,
	...
	*/
	...
	/** Set current view's X position and refresh display.
		@param posX New position that corresponds to the center point of the
	   view.
	*/
	...
	/** Set current view's Y position and refresh display.
		@param posY New position that corresponds to the center point of the
	   view.
	*/
	...
	*/
	...
	/** Set current view's dimensions in device context units.
		...
		@param scrX New position that corresponds to the center point of the
	   view.
		@param scrY New position that corresponds to the center point of the
	   view.
	*/
	 ...
	 * @sa p2y,x2p,y2p */
	//     double p2x(wxCoord pixelCoordX, bool drawOutside = true ); // {
	...
	*/
	...
	/** Set view to fit global bounding box of all plot layers and refresh
	   display. Scale and position will be set to show all attached mpLayers.
	...
	*/
	...
	size, and the pixel scales are computed accordingly. Also, in this case the
	passed borders are not saved as the "desired borders", since this use will
	be invoked only when printing.
	*/
	void Fit(
	...
	/** Counts the number of plot layers, excluding axes or text: this is to
	   count only the layers which have a bounding box. \return The number of
	   profiles plotted.
	*/
	unsigned int CountLayers();

	/** Counts the number of plot layers, whether or not they have a bounding
	   box. \return The number of layers in the mpWindow. */
	...
	bool SaveScreenshot(
		const wxString& filename, int type = wxBITMAP_TYPE_BMP,
		wxSize imageSize = wxDefaultSize, bool fit = false);

	/** This value sets the zoom steps whenever the user clicks "Zoom in/out" or
	 * performs zoom with the mouse wheel. It must be a number above unity. This
	 * number is used for zoom in, and its inverse for zoom out. Set to 1.5 by
	 * default. */
	static double zoomIncrementalFactor;

	/** Set window margins, creating a blank area where some kinds of layers
	   cannot draw. This is useful for example to draw axes outside the area
	   where the plots are drawn.
	...
	/** Gets coordinate tooltip status. \return true for enable, false for
	 * disable */
	...
	/** Check if a given point is inside the area of a mpInfoLayer and
	   eventually returns its pointer.
		...
		@return If an info layer is found, returns its pointer, nullptr
	   otherwise */
   ...
   protected:
	...
	/** Mouse handler, for detecting when the user drags with the right button
	 * or just "clicks" for the menu */
...
/** A class providing graphs functionality for a 2D plot (either continuous or a
   set of points), from vectors of data. This class can be used directly, the
   user does not need to derive any new class. Simply pass the data as two
   vectors with the same length containing the X and Y coordinates to the method
   SetData.
...
class WXDLLIMPEXP_MATHPLOT mpFXYVector : public mpFXY
{
	...
	*/
	...
	/** Changes the internal data: the set of points to draw.
		Both vectors MUST be of the same length. This method DOES NOT refresh
	  the mpWindow; do it manually.
	  * @sa Clear
	*/
 ...
 * location+rotation. The current transformation is set through
...
class WXDLLIMPEXP_MATHPLOT mpCovarianceEllipse : public mpMovableObject
{
   public:
	 ...
	 */
	mpCovarianceEllipse(
		double cov_00 = 1, double cov_11 = 1, double cov_01 = 0,
	...
	void GetCovarianceMatrix(
		double& cov_00, double& cov_01, double& cov_11) const
	{
		cov_00 = m_cov_00;
		cov_01 = m_cov_01;
		cov_11 = m_cov_11;
 ...
 * non-closed polygons by passing the appropriate parameters to "setPoints". To
 * draw a point-cloud, call "SetContinuity(false)".
 */
 ...
 * SetCoordinateBase. To ease the implementation of descendent classes,
 * mpMovableObject will be in charge of Bounding Box computation and layer
 * render, assuming that the object updates its shape in m_shape_xs &
 * m_shape_ys.
 */
